...
const EmployeeManagement = () => {
  ...
  const handleSave = async () => {
    try {
      // FIXED VALIDATION
      if (!employeeData.name || !employeeData.epfNo || !employeeData.nic) {
        alert("Please enter EPF No, Name and NIC.");
        return;
      }

      // UNIQUE CHECKS
      const existsEPF = employees.some(
        e => e.epfNo === employeeData.epfNo && e.id !== editingId
      );

      const existsNIC = employees.some(
        e => e.nic === employeeData.nic && e.id !== editingId
      );

      const existsPhone = employees.some(
        e => e.phone === employeeData.phone && e.id !== editingId
      );
    ...
    <div className="container" style={{ paddingTop: 20 }}>
      ...
      <div className="card">
        ...
        <div className="section-content">
          {selectedSection === "personal" && (
            <PersonalSection employeeData={employeeData} setEmployeeData={setEmployeeData} />
          )}
          {selectedSection === "job" && (
            <JobSection employeeData={employeeData} setEmployeeData={setEmployeeData} />
          )}
          {selectedSection === "salary" && (
            <SalarySection employeeData={employeeData} setEmployeeData={setEmployeeData} />
          )}
        </div>
      ...
      <div className="card" style={{ marginTop: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
          ...
          <input
            placeholder="Search EPF/Name/NIC"
            value={search}
        ...
        <table className="table">
          ...
          <tbody>
            {filtered.map(emp => (
              <tr key={emp.id}>
                ...
                <td>
                  {emp.profile_photo ? (
                    <img
                      src={`http://localhost:3005${emp.profile_photo}`}
                      alt="profile"
                      style={{
                        width: 40,
                        height: 40,
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} style={{ textAlign: "center" }}>No employees</td>
              </tr>
            )}
          </tbody>
        </table>
